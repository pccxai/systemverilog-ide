...
export function createContextBundleAudit(bundle = {}) {
  ...
  const audit = {
    ...
    xsimDiagnosticsEntryCount: bundle?.xsimDiagnostics?.summaryAvailable === true ? 1 : 0,
    ...
    safety: {
      ...
      networkCalls: false,
      mcpCalls: false,
      xsimDiagnosticsReadOnly: bundle?.xsimDiagnostics?.safety?.readOnly === true,
...
export function formatContextBundleAudit(audit) {
  return [
    ...
    `xsimDiagnosticsEntries: ${audit.xsimDiagnosticsEntryCount}`,
